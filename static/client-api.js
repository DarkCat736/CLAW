    ...
    service: {
        ...
        checklist: {
            init: async function () {
                ...
                await socket.timeout(2000).emit('checklist', {payload: "INIT", email: CLAW_ClientAPI.auth.user.email, password: CLAW_ClientAPI.auth.user.password}, async (err, response) => {
                    if (err) {
                        this.fatalError("Init error");
                    } else {
                        if (response.status === "error") {
                            this.fatalError(response.code);
                        } else {
                            await this.pullDBData(this.updateChecklistsList);
                            socket.on("checklist", (payload, callback) => {
                                let callbackMsg = this.messageDirector(payload, socket);
                                callback(callbackMsg);
                            });
                            console.log("Checklist init success");
                        }
                    }
                });
            },
            pullDBData: function(callback) {
                return new Promise(resolve => {
                    socket.timeout(10000).emit('checklist', {payload: "PULL_DB_DATA"}, (err, response) => {
                        if (err) {
                            this.fatalError("Pull db data error");
                            resolve(false);
                        } else {
                            if (response.status === "error") {
                                this.fatalError(response.code);
                                ...
                                CLAW_ClientAPI.service.checklist.data = response.payload;
                                console.log(CLAW_ClientAPI.service.checklist.data);
                                if (callback != null) {
                                    callback();
                                }
                                console.log("Checklist pull db data success");
                                resolve(true);
                            }
                        }
                    });
            ...
            pushDBData: function(callback) {
                return new Promise(resolve => {
                    socket.timeout(5000).emit('checklist', {payload: "PUSH_DB_DATA", data: CLAW_ClientAPI.service.checklist.data}, (err, response) => {
                        if (err) {
                            this.fatalError("Push db data error");
                            resolve(false);
                        } else {
                            if (response.status === "error") {
                                this.fatalError(response.code);
                                ...
                                console.log("Checklist push db data success");
                                resolve(true);
                            }
                        }
                    });
            ...
            updateChecklistsList: async function() {
                ...
                for (let i = 0; i < Object.keys(CLAW_ClientAPI.service.checklist.data).length; i++) {
                    ...
                    document.getElementById("menuOptionsContainer").innerHTML += `<p class="checklistMenuOption" id="checklistOption_${i}" onclick="CLAW_ClientAPI.service.checklist.switchChecklists(${i})" selected="false">${atob(CLAW_ClientAPI.service.checklist.data[i].title)}</p>`;
                }
            ...
            createNewChecklist: async function() {
                CLAW_ClientAPI.service.checklist.data[Object.keys(CLAW_ClientAPI.service.checklist.data).length] = {title: `${btoa("New checklist")}`, content: {0: {content: `${btoa("Add items now...")}`, completed: "false"}}};
            ...
            deleteChecklist: async function() {
                if (confirm(`Are you sure you want to delete the checklist "${atob(CLAW_ClientAPI.service.checklist.data[CLAW_ClientAPI.service.checklist.currentChecklistIndex].title)}"? This is an irreversible action.`)) {
            ...
            switchChecklists: async function(newIndex) {
                ...
                document.getElementById("checklistTitleText").innerHTML = atob(CLAW_ClientAPI.service.checklist.data[newIndex].title);
            ...
            fatalError: function(e) {
                alert(`This service has run into an unrecoverable error: ${e}\nPlease try starting the service again and make a GitHub issue if the problem persists.`);
                //window.open('/', '_self');
            },
            messageDirector: async function(payload, socket) {
                switch (payload.payload) {
                    default:
                        console.log("Failure to parse server message");
                }
            }
        },
        assignment_tracker: {
            init: async function() {
                await CLAW_ClientAPI.auth.init(true);
                //register with server
                await socket.timeout(2000).emit('assignment_tracker', {payload: "INIT", email: CLAW_ClientAPI.auth.user.email, password: CLAW_ClientAPI.auth.user.password}, async (err, response) => {
                    if (err) {
                        this.fatalError("Init error");
                    } else {
                        if (response.status === "error") {
                            ...
                            await this.pullAvailableClasses();
                            await this.pullAllClassesMetadata();
                            await this.uiUpdateClassList();
                            socket.on("assignment_tracker", (payload, callback) => {
                                let callbackMsg = this.messageDirector(payload, socket);
                                callback(callbackMsg);
                            });
                            console.log("Assignment tracker init success");
                        }
                    }
                });
            ...
            messageDirector: async function(payload, socket) {
                switch (payload.payload) {
                    case "PULL_CLASSES_COMMAND":
                        await this.pullAvailableClasses();
                        await this.pullAllClassesMetadata();
                        await this.uiUpdateClassList();
                        console.log("pulled classes on server command");
            ...
            pullAvailableClasses: async function() {
                return new Promise(resolve => {
                    socket.timeout(10000).emit('assignment_tracker', {payload: "GET_AVAILABLE_CLASSES"}, (err, response) => {
                        if (err) {
                            this.fatalError("Pull available classes error");
                            resolve(false);
                        } else {
                            if (response.status === "error") {
                                this.fatalError(response.code);
                                ...
                                this.localData.availableClasses = response.payload;
                                console.log("Assignment tracker pull available classes success");
                                resolve(true);
                            }
                        }
                    });
            ...
            pullAllClassesMetadata: async function() {
                //await this.pullAvailableClasses();
                return new Promise(async function (resolve){
                    for (let i = 0; i < Object.keys(CLAW_ClientAPI.service.assignment_tracker.localData.availableClasses).length; i++) {
                        await CLAW_ClientAPI.service.assignment_tracker.pullClassMetadata(i);
                    }
                    console.log("Assignment tracker pull class metadata success");
                    resolve(true);
                });
            },
            pullClassMetadata: async function(index) {
                return new Promise(resolve => {
                    socket.timeout(5000).emit('assignment_tracker', {payload: "GET_CLASS_METADATA", classId: CLAW_ClientAPI.service.assignment_tracker.localData.availableClasses[index].id}, (err, response) => {
                        if (err) {
                            this.fatalError("Pull class metadata error");
                            resolve(false);
                        } else {
                            if (response.status === "error") {
                                this.fatalError(response.code);
                                ...
                                console.log(response.payload);
                                CLAW_ClientAPI.service.assignment_tracker.localData.classesMetadata[index] = response.payload;
                                resolve(true);
                            }
                        }
                    });
            ...
            createNewClass: async function() {
                return new Promise(resolve => {
                    socket.timeout(5000).emit('assignment_tracker', {payload: "CREATE_NEW_CLASS"}, (err, response) => {
                        if (err) {
                            this.fatalError("Create class error");
                            resolve(false);
                        } else {
                            if (response.status === "error") {
                                this.fatalError(response.code);
                                ...
                                console.log(response.payload);
                                console.log("Assignment tracker create class success");
                                resolve(true);
                            }
                        }
                    });
            ...
            uiUpdateClassList: async function() {
                console.log("updating UI: ", this.localData.classesMetadata);
                document.getElementById("classes_menu").innerHTML = `<p class="menuTitle">Class: </p><button class="menuOption">All</button>`;
                for (let i = 0; i < Object.keys(this.localData.classesMetadata).length; i++) {
                    document.getElementById("classes_menu").innerHTML = document.getElementById("classes_menu").innerHTML + `<button class="menuOption">${this.localData.classesMetadata[i].name}</button>`;
                    console.log(this.localData.classesMetadata[i].name, i);
                }
                document.getElementById("classes_menu").innerHTML += `<button class="menuOption" onclick="CLAW_ClientAPI.service.assignment_tracker.createNewClass()"><i>New Class...</i></button>`;
            },
            localData: {
                classesMetadata: {
